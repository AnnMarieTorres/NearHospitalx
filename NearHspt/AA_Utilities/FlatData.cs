...
namespace NearHspt.AA_Utilities
{
  class FlatData
  {
    ...
    // ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    // Get hospital "DB". One use, in App
    //
    //
    // 00 - biz_name
    // 01 - biz_info
    ...
    public static bool GetHospitalDB()
    {
      ...
      try
      {
        var assembly = typeof(FindHospital).GetTypeInfo().Assembly;
        foreach (var res in assembly.GetManifestResourceNames())
        {
          if (res.Contains("kranken.txt"))
          {
    ...
    // ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    // Load all hospitals in range, one use, in FindHospital
    //
    ...
    // 2. if < 100 miles save in index array <hospital pointer, distance>
    ...
    public static void Load_HospitalsInRage(double inLatfrom, double inLongfrom)
    {
      ...
      if (inLatfrom <= 0.0) iB = true;
      //if (inLongfrom <= 0.0) iB = true;
      if (iB)
      {
      ...
      try
      {
        ...
        {
          //
          // Some records have no Latitude / Longitude
          // both cases will be handled by the abbreviated catch {}
          try
          {
            // Get 'radian' distance, i.e. "Bird view" distance between device-address and array entry
            ...
            // convert actual distance to int, multiply by 100 to capture last two digits ( 45.78 --> 4578 )
            intdistanceInRadius = Convert.ToInt32((distanceInRadius) * 100);
            // compare 4578 to default distance converted to int and multiplied by 100 ( 100.00 -- > 10000 )
            int intRange = (Convert.ToInt32(App.hospitalRange) * 100);
            if (intdistanceInRadius < intRange)
            {
      ...
      if (iB == false)
      {
        // the sort is by distance
        FlatData.SorthospitalsInRange();
        if (App.hospitalsInRangeCount > 100) App.hospitalsInRangeCount = 100;
      }
    }
